...
func ProvisionAccountHandler(c *gin.Context) {
	...
	clientEmail, _ := sess.Values["email"].(string) // Usar email como clientName
	if !ok || userID == "" {
	...
	log.Printf("Iniciando provisionamento para o usuário %s. Domínio: %s, Plano WHM: %s, Usuário cPanel: %s", userID, p.Domain, whmPlanName, username)

	if whm.WhmClient != nil {
		resp, err := whm.WhmClient.CreateAccount(p.Domain, username, password, whmPlanName, clientEmail)
		if err != nil {
		...
		log.Printf("AVISO: Simulando provisionamento para o domínio %s (cliente WHM não configurado)", p.Domain)
	...
	serviceData := map[string]interface{}{
		"id":          serviceID,
		"clientId":    userID,
		"clientName":  clientEmail, // Usar o email para consistência
		"serviceType": p.Plan,
